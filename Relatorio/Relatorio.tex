...
% \qquad Na literatura, usualmente utiliza-se a nota\c{c}\~ao $\mathcal{O}$ para formalizar esses conceitos.
% \'E sabido que o n\'umero de compara\c{c}\~oes do Insertion Sort est\'a em
% $\mathcal{O}(n^{2})$ e o do Merge Sort est\'a em $\mathcal{O}(n\log(n))$, considerando listas de tamanho $n$.
% Isso significa que, existem constantes $c_{1}$ e $c_{2}$ tais que o primeiro n\'umero \'e sempre menor que
% $c_{1}n^{2}$ e o segundo \'e sempre menor que $c_{2}n\log(n)$. Nesse sentido, os resultados obtidos neste
% trabalho concordam com a literatura, al\'em de apresentarem limites mais precisos.
